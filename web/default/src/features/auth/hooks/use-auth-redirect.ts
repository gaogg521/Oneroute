...
const ADMIN_PATHS = ['/admin-teamv3', '/channels', '/users', '/redemption-codes', '/system-settings', '/models']

function isSafeRedirect(path: string | undefined, userRole: number): boolean {
  if (!path) return false
  const isAdminPath = ADMIN_PATHS.some(p => path.startsWith(p))
  if (isAdminPath && userRole < ROLE.ADMIN) return false
  return true
}
...
export function useAuthRedirect() {
  ...
  const handleLoginSuccess = async (
    ...
    // Navigate to target page (fall back to dashboard if redirect requires higher role)
    const targetPath = isSafeRedirect(redirectTo, userRole) ? redirectTo! : '/dashboard'
