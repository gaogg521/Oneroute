...
const paymentSchema = z.object({
  ...
  AntomPrivateKey: z.string(),
  AntomPublicKey: z.string(),
  AntomGateway: z.string().refine((value) => {
    const trimmed = value.trim()
    if (!trimmed) return true
    return /^https?:\/\//.test(trimmed)
  }, 'Provide a valid URL starting with http:// or https://'),
  AntomPaymentMethodType: z.string(),
...
export function PaymentSettingsSection({
  ...
  const onSubmit = async (values: PaymentFormValues) => {
    const sanitized = {
      ...
      AntomPrivateKey: values.AntomPrivateKey.trim(),
      AntomPublicKey: values.AntomPublicKey.trim(),
      AntomGateway: removeTrailingSlash(values.AntomGateway),
      AntomPaymentMethodType: values.AntomPaymentMethodType.trim(),
    ...
    const initial = {
      ...
      AntomPrivateKey: initialRef.current.AntomPrivateKey.trim(),
      AntomPublicKey: initialRef.current.AntomPublicKey.trim(),
      AntomGateway: removeTrailingSlash(initialRef.current.AntomGateway),
      AntomPaymentMethodType: initialRef.current.AntomPaymentMethodType.trim(),
    ...
    if (
      sanitized.AntomPrivateKey &&
      sanitized.AntomPrivateKey !== initial.AntomPrivateKey
    ) {
      updates.push({ key: 'AntomPrivateKey', value: sanitized.AntomPrivateKey })
    }
    ...
    if (sanitized.AntomPaymentMethodType !== initial.AntomPaymentMethodType) {
      updates.push({
        key: 'AntomPaymentMethodType',
        value: sanitized.AntomPaymentMethodType,
      })
    }
            ...
            <TabsContent value='antom' className={paymentTabContentClassName}>
              <div className='space-y-4'>
                ...
                <div className='rounded-md bg-blue-50 p-4 text-sm text-blue-900 dark:bg-blue-950 dark:text-blue-100'>
                  ...
                  <ul className='list-inside list-disc space-y-1'>
                    <li>
                      {t('Notify URL:')}{' '}
                      <code className='rounded bg-blue-100 px-1 py-0.5 text-xs dark:bg-blue-900'>
                        {'<ServerAddress>/api/antom/notify'}
                      </code>
                    </li>
                    <li>{t('Configure this notify URL in the Antom Dashboard')}</li>
                  </ul>
                </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder='SANDBOX_5X00000000000000'
                            autoComplete='off'
                            {...field}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder='https://open-sg-global.alipay.com'
                            {...field}
                            onChange={(event) => field.onChange(event.target.value)}
                          />
                        </FormControl>
                        <FormDescription>
                          {t('Antom API endpoint base URL')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          rows={3}
                          placeholder={t('Enter new key to update (PKCS#8 base64)')}
                          autoComplete='new-password'
                          {...field}
                          onChange={(event) => field.onChange(event.target.value)}
                        />
                      ...
                      <FormDescription>
                        {t('Your RSA private key used to sign requests')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Antom public key')}</FormLabel>
                      <FormControl>
                        <Textarea
                          rows={3}
                          placeholder={t('Antom public key (X.509 base64)')}
                          {...field}
                          onChange={(event) => field.onChange(event.target.value)}
                        />
                      </FormControl>
                      <FormDescription>
                        {t('Used to verify Antom responses and notifications')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-4'>
                  <FormField
                    control={form.control}
                    name='AntomPaymentMethodType'
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('Payment method type')}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder='ALIPAY_CN'
                            {...field}
                            onChange={(event) => field.onChange(event.target.value)}
                          />
                        </FormControl>
