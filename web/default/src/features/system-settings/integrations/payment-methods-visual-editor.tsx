...
export function PaymentMethodsVisualEditor({
  ...
  const handleToggleEnabled = (method: PaymentMethodData) => {
    const parsed = safeJsonParseWithValidation<unknown[]>(value, {
      fallback: [],
      validator: isArray,
      silent: true,
    ...
    const updatedArray = parsed.map((item) => {
      if (
        ...
        'name' in item &&
        'type' in item &&
        (item as PaymentMethodData).name === method.name &&
        (item as PaymentMethodData).type === method.type
      ) {
        return { ...(item as PaymentMethodData), enabled: !(method.enabled ?? true) }
      }
      return item
    })
      ...
      {filteredMethods.length === 0 ? (
        ...
        <div className='rounded-md border'>
          ...
          <StaticDataTable
            ...
            getRowClassName={(method) =>
              method.enabled === false ? 'opacity-50' : undefined
            }
            columns={[
              {
                id: 'enabled',
          ...
          <div className='divide-y md:hidden'>
            {filteredMethods.map((method) => {
              ...
              return (
                <div
                  key={methodKey}
                  className={`p-4${method.enabled === false ? ' opacity-50' : ''}`}
                >
