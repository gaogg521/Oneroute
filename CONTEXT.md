...
| 日期 | 模块 | 内容 |
|------|------|------|
| 2026-07-20 | 豆包(火山)图片/视频/3D 中转排障+修复+3D支持 | 用户接入火山引擎(VolcEngine)豆包模型，文本通过但图片/视频测试失败。**排查三类模型走三条链路**：文本→`/api/v3/chat/completions`(VolcEngine渠道type45)；图片seedream→`/api/v3/images/generations`(同VolcEngine渠道)；视频seedance/3D→`/api/v3/contents/generations/tasks`异步任务(DoubaoVideo渠道type54)。**关键发现**：`relay_adaptor.go:157` 里 `ChannelTypeDoubaoVideo(54)` 和 `ChannelTypeVolcEngine(45)` **都路由到同一个 `taskdoubao.TaskAdaptor`**。**修复1·图片测试bug**(commit `7c5f3260`)：`controller/channel-test.go` 的 `buildTestRequest` 自动检测分支漏了图片模型——requestPath已判成`/v1/images/generations`且relayFormat=OpenAIImage(RelayMode=ImagesGenerations)，但仍构造成`GeneralOpenAIRequest`，导致`request.(*dto.ImageRequest)`断言失败报"invalid image request type"。补上 `channel.Type==VolcEngine && strings.Contains(model,"seedream")` 分支返回`ImageRequest`(镜像line145 requestPath判定)。**修复2·3D支持**(commit `cd1769ee`)：豆包任务适配器 `relay/channel/task/doubao/adaptor.go` 原只从 `content.video_url` 取产物，3D(doubao-seed3d)产物在**另一字段**导致取空→退回无效代理URL。**用用户真实火山Key直连上游跑通真实3D任务确认**：3D产物在 `content.file_url`(含.obj的.zip)、`usage.total_tokens=30000`。改法：`responseTask.Content` 加 `FileURL string json:"file_url"`；新增 `resultURL()` helper(优先video_url，回退file_url)；`ParseTaskResult`+`ConvertToOpenAIVideo` 改用该helper；`constants.go` ModelList 加 `doubao-seed3d-2-0-260328`；`adaptor_test.go` 加3D file_url用例。**验证**：`go build`+`go test ./relay/channel/task/doubao/...` 全过；**真实上游+网关端到端双验证**——直连火山视频(seedance-2-0)成功返回mp4、3D(seed3d)成功返回.zip；部署后用生产网关 `ai.oneroute.vip` POST `/v1/video/generations` 跑通视频(result_url=mp4)和3D(result_url=.obj的zip)，计费(quota扣减+成功后token重算差额退款)均正常。**答疑澄清**：①游乐场只调 `/pg/chat/completions` 纯对话，测不了图片/视频生成(非bug)；②渠道"测试"按钮对异步任务渠道(MJ/Suno/Kling/Jimeng/**DoubaoVideo**/Vidu，见`channel-test.go:86-100`黑名单)一律返回"channel test is not supported"，是设计如此非故障；③图片"RPM exceeded"是火山上游对seedream的限流，非网关问题；④视频最初报500是seedance放错在VolcEngine渠道被当对话请求，改放DoubaoVideo渠道即可。**计费坑提醒**：`model_ratio.go:403` 未配置模型兜底倍率=37.5(极高，约$75/百万token)，seedance/seed3d 未配价时按此兜底多扣(约真实价12倍)；`task/doubao/constants.go` videoPriceTable 记录真实基准价 seedance-2-0=46元/百万token、fast=37元，应据此配ModelRatio。**两处部署**：均按规则4 scp源码→服务器`go build -o new-api.deploy`→备份`new-api.bak.20260716`/`.20260720`→`mv`替换→`systemctl restart oneroute`→站点200。**⚠️遗留**：排障中用户在对话里明文贴了火山API Key(`3bff...e4fb`)和网关令牌(`sk-vwZ...`)，已多次提醒用户去各自控制台轮换。**未做**：seedance/seed3d 的真实价格配置由用户自行在后台完成(业务决策)，本次只提供基准价参考。 |
| 2026-07-14 | 支付网关·汇率/货币配置答疑+实测 | 用户对「货币与展示→USDExchangeRate」「支付网关→通用设置 Price」「各网关(OneOne/Stripe/Antom/Waffo)独立 UnitPrice」三层汇率概念提出连环疑问，逐条代码溯源确认：①`USDExchangeRate` 仅影响额度**展示**（`GetUsdToCurrencyRate`/`LogQuota`），切 USD 模式时完全不读该值；②`Price`(Epay/WaffoPancake共用)与各网关独立`XxxUnitPrice`互不联动，需分别改；③充值页预设按钮"Pay X"用前端 `status.price` 缓存(`useStatus` staleTime 5min)现算，自定义金额"待支付金额"是实时调后端算，改价后短暂不一致是缓存滞后非bug。用户对 OneOne「货币」纯文本框(无枚举校验)是否安全打错追问"你帮我测试一下"——**本地起 PORT=3000 后端(已有 `new-api.exe`，生产库镜像)+`bunx rsbuild dev --port 5173`前端**（临时 `.env.local` 设 `VITE_REACT_APP_SERVER_URL`，验证后已删除；本地`zhaogao`密码因上次会话(0710)已丢失原哈希且未记录新值，本次重置为**bcrypt(TestLocal123!)**，同样未来若需登录本地库请自行改密或告知）。**实测发现一个真实的结构性不一致**（非本次追问的"打错字符串"场景，而是更严重的价格来源分裂）：`usePayment.calculatePaymentAmount`(`use-payment.ts:56-62`)只对 Stripe/WaffoPancake 特判，Antom 和 OneOne 都落到通用 `calculateAmount`→后端 `RequestAmount`(`topup.go:600`)→`getPayMoney`（读全局 `Price`），但真正下单走的是 `requestOneOnePayment`→`getOneOnePayMoney`（读该网关自己的 `OneOneUnitPrice`）——**只要 `OneOneUnitPrice`(或`AntomUnitPrice`)与全局`Price`不同，用户在充值页选 OneOne/Antom 时看到的"待支付金额"预览永远是按 Epay 的价格算的，和点击后真正走 OneOne/Antom 网关下单时算的金额不一致**，不是缓存问题，是从未接线。用真实本地数据复现：`Price`=7、临时把 `OneOneUnitPrice` 改成6，`/api/user/amount`(充值500) 预览仍返回 3500.00（=500×7，纹丝不动），而按 `getOneOnePayMoney` 公式真正会算出 3000（=500×6）——两者必然不同步。**测试完已还原**：`OneOneCurrency`→`USD`、`OneOneUnitPrice`→`1`（改回原值），停掉本地后端+前端dev进程，删除临时`.env.local`。**未做**：未真实调用 OneOne/Antom 线上网关下单（需真实商户凭证，会在其后台留下订单记录，未经用户同意不做）。**已修复并上线**（同日追加）：新增 `RequestAntomAmount`/`RequestOneOneAmount`(`controller/topup_antom.go`/`topup_oneone.go`，镜像 `RequestWaffoAmount` 写法，仅算金额不下单)，路由注册 `/antom/amount`/`/oneone/amount`(`router/api-router.go`，不加限流，与其余 `/amount` 系列一致)；前端 `wallet/api.ts` 新增 `calculateAntomAmount`/`calculateOneOneAmount`，`use-payment.ts` 的 `calculatePaymentAmount` 补上 isAntom/isOneOne 分支改调这两个新函数。**本地验证**：复现之前场景(`Price=7`/`OneOneUnitPrice=6`/`AntomUnitPrice=3`)，充值500选 OneOne 预览从错误的 3500 变成正确的 3000（网络面板确认走的是新 `/api/user/oneone/amount`，非旧的通用 `/api/user/amount`）；直接调 `/api/user/antom/amount` 验证返回 1500（=500×3，同样正确）。验证完把 `OneOneUnitPrice` 改回 1（`Price`=7、`AntomUnitPrice`=3 本身就是环境已有值，未改动）。`go build`+`bun run typecheck`+`bun run build` 均干净。**已提交**(`5d0491ce`)并 `push origin main`，**已部署生产**：scp 5个源码文件(md5校验一致)→服务器 `bun run build`(新bundle `index.a1b41a05d5.js`)+`go build -o new-api.deploy .`→备份旧二进制`mv`替换→`systemctl restart oneroute`→日志干净、`https://ai.oneroute.vip/` 200、新路由未登录态返回401(非404，证明已生效)。**未做**：未用真实凭证走一遍 Antom/OneOne 真实下单端到端支付（会产生真实网关订单，超出本次预览接口修复范围）。 |
