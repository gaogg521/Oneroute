...
- [ ] **Antom/OneOne 充值预览金额不接线**：充值页选 Antom/OneOne 时，"待支付金额"预览调用的是 Epay 通用 `/api/user/amount`（读全局 `Price`），但真正下单走各网关自己的 `AntomUnitPrice`/`OneOneUnitPrice`——两个费率不同时预览和实付永远对不上，非缓存问题。需要给 `calculatePaymentAmount`(`use-payment.ts`) 补上 isAntom/isOneOne 分支调各自的 amount 预览端点（详见 2026-07-14 记录）
