...
type RelayInfo struct {
	...
	// TeamOwnerId is non-zero when the relay user is a team member (non-owner).
	// Billing session will deduct from the team owner's wallet instead of the member's.
	TeamOwnerId int
	// TeamMemberQuotaLimit is this member's per-member spending cap (0 = unlimited).
	TeamMemberQuotaLimit int
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	info := &RelayInfo{
		...
		TeamOwnerId:          common.GetContextKeyInt(c, constant.ContextKeyTeamOwnerId),
		TeamMemberQuotaLimit: common.GetContextKeyInt(c, constant.ContextKeyTeamMemberQuotaLimit),
