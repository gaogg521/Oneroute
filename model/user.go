...
func GetSubordinateUsers(inviterId int, pageInfo *common.PageInfo) ([]*User, int64, error) {
	tx := DB.Begin()
	if tx.Error != nil {
		return nil, 0, tx.Error
	}
	...
	err = tx.Where("inviter_id = ?", inviterId).Order("id desc").Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func AdminGetTeamLeaders(keyword string, offset, limit int) ([]*TeamLeader, int64, error) {
	var leaders []*TeamLeader
	var total int64

	// Subquery to find distinct inviter_ids of active users who have at least one subordinate
	subQuery := DB.Model(&User{}).Select("DISTINCT inviter_id").Where("inviter_id != 0")
	...
	var users []*User
	if err := query.Order("id desc").Offset(offset).Limit(limit).Scan(&users).Error; err != nil {
		return nil, 0, err
	}

	leaders = make([]*TeamLeader, 0, len(users))
	for _, u := range users {
		leader := &TeamLeader{
			Id:          u.Id,
			Username:    u.Username,
		...
		// Count members
		var count int64
		DB.Model(&User{}).Where("inviter_id = ?", u.Id).Count(&count)
		leader.MemberCount = int(count)

		// Calculate total used quota of members
		var result struct {
			TotalUsed int
		}
		var memberIds []int
		DB.Model(&User{}).Where("inviter_id = ?", u.Id).Pluck("id", &memberIds)
		if len(memberIds) > 0 {
			LOG_DB.Model(&Log{}).
				Select("COALESCE(SUM(quota), 0) as total_used").
				Where("user_id IN ? AND type = ?", memberIds, LogTypeConsume).
				Scan(&result)
			leader.TotalUsedQuota = result.TotalUsed
		}
