...
// RechargeAntom completes a pending Antom (Alipay Global) topup order and
// credits the user. Mirrors Recharge (Stripe): quota = Money * QuotaPerUnit.
// Idempotent via row lock + pending-status guard.
func RechargeAntom(referenceId string, callerIp string) (err error) {
	if referenceId == "" {
		return errors.New("未提供支付单号")
	}
