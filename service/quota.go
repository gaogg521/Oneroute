...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	// Team billing: check owner's balance and per-member quota limit if applicable.
	billingUserId := relayInfo.UserId
	if relayInfo.TeamOwnerId > 0 {
		billingUserId = relayInfo.TeamOwnerId
		if relayInfo.TeamMemberQuotaLimit > 0 {
			...
			if usedQuota >= relayInfo.TeamMemberQuotaLimit {
				return fmt.Errorf("团队成员额度已达上限（上限 %s，已用 %s）",
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		// Wallet — redirect to team owner when applicable
		walletUserId := relayInfo.UserId
		if relayInfo.TeamOwnerId > 0 {
			walletUserId = relayInfo.TeamOwnerId
		}
		if quota > 0 {
			err = model.DecreaseUserQuota(walletUserId, quota, false)
		} else {
			err = model.IncreaseUserQuota(walletUserId, -quota, false)
		}
		if err != nil {
			return err
		}
