...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	// Team billing: if this token's user is a non-owner team member, record the
	// owner's userId so billing_session can redirect wallet deductions there.
	if ownerId, quotaLimit, found := model.GetTeamOwnerForMember(token.UserId); found && ownerId != token.UserId {
		common.SetContextKey(c, constant.ContextKeyTeamOwnerId, ownerId)
		common.SetContextKey(c, constant.ContextKeyTeamMemberQuotaLimit, quotaLimit)
	}
